...
def build_instance(region: str,
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'InvalidGroup.NotFound':
            ...
            # Enable SSH to the C2 server
            ec2.authorize_security_group_ingress(GroupName=group_name,
                                                 IpProtocol='tcp',
                                                 FromPort=22,
                                                 ToPort=22,
                                                 CidrIp='0.0.0.0/0')
            # Enable HTTP to the server for hosting files and comms
            ec2.authorize_security_group_ingress(GroupName=group_name,
                                                 IpProtocol='tcp',
                                                 FromPort=80,
                                                 ToPort=80,
                                                 CidrIp='0.0.0.0/0')
            # Enable HTTPS to the server for hosting files and comms
            ec2.authorize_security_group_ingress(GroupName=group_name,
                                                 IpProtocol='tcp',
                                                 FromPort=443,
                                                 ToPort=443,
                                                 CidrIp='0.0.0.0/0')
            # Enable HTTP Proxy for tunneling when/if needed
            ec2.authorize_security_group_ingress(GroupName=group_name,
                                                 IpProtocol='tcp',
                                                 FromPort=8080,
                                                 ToPort=8080,
    ...
    print("You can now login to the machine with ssh at the following"
          "IP address: %s" % instance['Instances'][0]['PublicIpAddress'])
...
def configure_instance(c2: str,
    ...
    key_name (str): Path of the SSH key to use for configuring the instance
    '''
    ...
    std_input, std_output, std_error = ssh_client.exec_command("sudo apt-get "
                                                               "update && "
                                                               "sudo apt-get "
                                                               "install %s" %
                                                               (c2),
...
def main():
    ...
    if args.build:
        build_instance(region=args.REGION,
                       ami=args.AMI,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=('Creates AWS EC2 instances '
                                                  'for on-demand C2 testing.'))
    parser.add_argument('-r', '--region',
                        type=str,
                        dest='REGION',
                        default="us-east-1",
                        help="AWS region for deployment: us-east-1")
    parser.add_argument('-i', '--ami',
                        type=str,
                        dest='AMI',
                        default="ami-06fd113e1286dd166",
                        help="AMI image id: ami-06fd113e1286dd166")
    parser.add_argument('-k', '--keyname',
                        type=str,
                        dest='key_name',
                        default="aws_c2",
                        help="SSH keyname: aws_c2")
    parser.add_argument('-n', '--createkey',
                        action='store_true',
                        help="Create new AWS ssh keys: False")
